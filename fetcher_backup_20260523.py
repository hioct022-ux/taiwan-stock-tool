        ...
        if data.get('stat') == 'OK':
            for row in data.get('data', []):
                try:
                    code = row[0].strip()
                    chips = {
                        'foreign_buy':  clean_num(row[2]),
                        'foreign_sell': clean_num(row[3]),
                        'foreign_net':  clean_num(row[4]),
                        'trust_buy':    clean_num(row[11]),
                        'trust_sell':   clean_num(row[12]),
                        'trust_net':    clean_num(row[13]),
                        'dealer_buy':   clean_num(row[5]),
                        'dealer_sell':  clean_num(row[6]),
                        'dealer_net':   clean_num(row[7]),
                        'margin_balance': 0,
                        'short_balance':  0,
                    }
                ...
                for row in data.get('data', []):
                    try:
                        if row[0].strip() != code:
                            continue
                        # 取得日期
                        date_tw = data.get('date', date_str)
                        date_std = f'{int(date_tw[:3])+1911}-{date_tw[3:5]}-{date_tw[5:7]}' if len(date_tw) == 8 else datetime.now().strftime('%Y-%m-%d')
                        chips = {
                            'foreign_buy':  clean_num(row[2]),
                            'foreign_sell': clean_num(row[3]),
                            'foreign_net':  clean_num(row[4]),
