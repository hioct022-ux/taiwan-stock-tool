...
**⚠️ 儀表板基準與實際執行規則可能不一致（2026-08 發現，尚未處理）：** 頂部四格 metric 的 `vs 回測` 差值寫死用**策略C**的基準（60.6%／+10.20%，見 `app.py` 約 8071–8080 行）。但使用者實際操作若走「大盤轉空減碼」提前出場，跑的其實是**策略D**的邏輯，而 D 自己的基準本來就低於 C。等於拿 C 的尺量 D 的結果，即使完美執行 D 也會兩格全紅、看起來像「策略失效」，可能誘使去動其實沒問題、且已用 404 筆驗證過的進場門檻或評分邏輯——真正的差異只在**出場規則**這一項。目前的緩解是儀表板本來就有「依出場原因分組」，可只看「大盤轉空」那組去對照 D。**尚未改介面**（可選做法：caption 同時列 C／D 兩組基準、加基準切換、或依「大盤轉空出場佔比」自動選基準），理由是目前已平倉樣本僅個位數，任何數字都還在雜訊範圍，知道這件事比改程式重要，待累積約 20 筆再決定。
