...
`fetch_futures_institutional()`（台指期法人）、`fetch_options_pc()`（選擇權P/C）、`fetch_market_pe()`（大盤本益比）**從來沒有智慧補齊機制**（陷阱39 已載明「非本次範圍」）。每漏一天就永久少一天，其中大盤本益比最嚴重（近30天缺12天）。

**現在至少會被看見了**——這正是本章的目的。要根治需比照陷阱39 的模式，各自加上「查DB最新日期→補齊缺口→抓今天」的迴圈，但那三支 API 是否支援指定歷史日期需先逐一驗證（**且必須驗證回傳日期＝要求日期，見陷阱41 的教訓**）。
