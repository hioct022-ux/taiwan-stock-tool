...
def check_conditions(ind, chips_list, close, market_chg_5d=None):
    ...
    if ma20 and close:
        if close < ma20:
            sell_conditions.append({
                'status': 'warn',
                'text': f'🔴 出場訊號1：收盤價（{close}元）已跌破MA20（{ma20}元），需觀察是否連續3日低於MA20'
            })
        else:
            sell_conditions.append({
                'status': 'pass',
                'text': f'🟢 安全1：收盤價（{close}元）仍在MA20（{ma20}元）上方，持股安全'
            })
    ...
    if rsi:
        if rsi > 80:
            sell_conditions.append({
                'status': 'warn',
                'text': f'🔴 出場訊號2：RSI(14)={rsi} 已超過80，短期漲幅過快，考慮減碼'
            })
        else:
            sell_conditions.append({
                'status': 'pass',
                'text': f'🟢 安全2：RSI(14)={rsi} 未超過80，短期動能正常'
            })
    ...
    if chips_list and len(chips_list) >= 5:
        ...
        if consecutive_sell and abs(total_sell5) > 30000:
            sell_conditions.append({
                'status': 'warn',
                'text': f'🔴 出場訊號3：外資連續5日賣超合計 {total_sell5:,} 張（超過3萬張），法人明顯撤退'
            })
        else:
            sell_conditions.append({
                'status': 'pass',
                'text': f'🟢 安全3：外資未連續大量賣超，籌碼無明顯撤退'
            })
    ...
    if stop_loss and close:
        if close <= stop_loss:
            sell_conditions.append({
                'status': 'warn',
                'text': f'🔴 出場訊號4：股價（{close}元）已跌破停損價（{stop_loss}元），建議執行停損保護資金'
            })
        else:
            sell_conditions.append({
                'status': 'pass',
                'text': f'🟢 安全4：股價（{close}元）未跌破停損價（{stop_loss}元），持股安全'
            })
